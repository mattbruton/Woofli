...
namespace woofli_be_v2._0.Controllers
{
    public class PetsitterController : ApiController
    {
        ...
        {
            if (_repo.GetAllPetsittersForUser(FindActiveUserName()).Any(p => p.PetsitterId == id))
            {
                return _repo.GetPetsitterById(id);
            }
            return null;
        }
        ...
        public Dictionary<string, bool> Delete(int id)
        {
            ...
            {
                _repo.RemovePetsitterById(FindActiveUserName(), id);
